...
\begin{frame}{Previously on FMM}
	...
	\begin{itemize}
		...
		\begin{itemize}
			\item \structure{Speculative view}: buy low/sell high speculator makes `roundtrips' in the asset, and therefore pays the spread
			\item \structure{Portfolio view}: investors are hit by liquidity shocks and must engage in costly adjustments
		\end{itemize}
...
\begin{frame}{Access to capital}
	\begin{itemize}
		\item Initial public offering (IPO): first time a firm gets listed on an exchange
		\begin{itemize}
			...
			\item Many reasons, but liquidity (due to asymm info) seems to be a factor: (\cite{ellul_ipo_2006})
...
\begin{frame}{Shareholders and governance}
	\begin{itemize}
		...
		\end{itemize}
		\pause 
		\item But what are \structure{investors' motives} when buying stocks? Do they want to engage in such oversight?
		\begin{itemize}
...
\begin{frame}{Shareholders and governance}
	\begin{itemize}
		...
		\end{itemize}
		\item If a manager makes bad decisions, a \emph{large} shareholder may (at some private cost) seek to \structure{improve the governance}
		\begin{itemize}
...
\begin{frame}{Information}
	\begin{itemize}
		...
		\end{enumerate}
		\only<2|handout:0>{
			\item How often would you say the market has superior information compared to firm?
		}
		\pause[3]
...
\begin{frame}
	\centering
	\includegraphics[height=\textheight]{pics/crowd}
	% https://www.smbc-comics.com/comic/efficient-3
\end{frame}
