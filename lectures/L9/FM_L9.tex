...
\begin{frame}[label=ideas]
	...
	\begin{itemize}
		...
		\item \hyperlink{example}{\beamerbutton{An (extreme) example of poorly informed trading}}
	\end{itemize}
\end{frame}
...
\begin{frame}{Search costs}
	Idea based on \cite{diamond_model_1971}'s \structure{chain store paradox}
	\begin{itemize}
...
\begin{frame}{Search costs}
	\begin{itemize}
		...
		\end{itemize}
		\item \structure{Welfare} implications of search costs:
		\begin{itemize}
...
\begin{frame}{Quote transparency}
	\begin{itemize}
		...
		\end{itemize}
		\item ...\structure{depth} can be more difficult to gauge
...
\begin{frame}[label=quotes]{Uncertainty and price sensitivity}
	\begin{itemize}
		...
		\begin{align*}
			...
			\mathbb{E} [x^T] &> x^O
		\end{align*}
		\item \structure{More (informed) trading in transparent market}
		\begin{itemize}
...
\begin{frame}{Order flow: Model}
	\begin{itemize}
		...
		\end{itemize}
		\item \structure{Idea}: higher order flow correlation  when traders are informed. Intuition: 
		\begin{itemize}
...
\begin{frame}{Order flow: Equilibrium}
	\begin{itemize}
		...
		\begin{itemize}
			...
			%Note: informed traders may prefer to quit the market, but this is only in this ``extreme'' version of the model
		\end{itemize}
	\end{itemize}
\end{frame}
...
\begin{frame}{Post-trade transparency: Period 2}
	...
	\begin{itemize}
		...
		\begin{itemize}
			...
			\item \structure{Dealer $I$}: Suppose you saw the first trade, and second trade is a buy, and $U$-dealer quotes $a_2^U$:
			\begin{itemize}
				...
				%\item $U$ wins period-2 buy order if $d_1$ was a buy, since $I$ knows that asset value is high
			\end{itemize}
...
\begin{frame}
	...
	\begin{itemize}
		...
		%\item ``Corporates, funds, and non-bank financial firms are always considered to be price takers and are a subgroup of the total aggregate buy side. Banks acting as market makers are always reported on the sell side.''
