		...
		\begin{column}{0.7\linewidth}
			{\setstretch{1.3}
				\begin{itemize}
					...
					\only<4|handout:1>{
					\item Depending on semantics:
					\begin{itemize}
						\item \structure{arbitrage in nominal prices} cannot be realized due to limited liquidity...
						\item (also, arbitrage itself is costly, due to leverage and short-selling constraints)
						\item ...so there is \alert{no real arbitrage}
					\end{itemize}
					}
				\end{itemize}
			}
...
\begin{frame}{Conclusion}
	\begin{itemize}
		\item Empirical research finds evidence on both a \structure{liquidity premium} and a \structure{liquidity risk premium} on stocks
