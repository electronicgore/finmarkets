...
\begin{frame}{Determinants of liquidity}
	\begin{itemize}[<+->]
		...
		\item Source of illiquidity: \visible<handout:0>{\alert{market thinness}} (too few traders present in the market at a given time). Or so I told you...
		\item If that was the only reason, trivial solutions for efficiency:
		\begin{itemize}
			\item \visible<handout:0>{coordinate on a time (that's how farmers' markets work!)}
			\item or \visible<handout:0>{add many dealers (that's how many markets work!)}
			\item This narrows down the spread, but does not close it.
		\end{itemize}
		\item So \structure{what drives the spread}? (apart from market thinness)
		\begin{itemize}
...
\begin{frame}{Price determinants}
	\begin{columns}
		\begin{column}{0.5\linewidth}
			{\setstretch{1.3}
				\begin{itemize}
					\item Stock prices move around all the time
					\item Media reports are full of ex post rationalizations...
					%\item Look at CNN Money reports (on absalon)
					\item ...but these can be pretty arbitrary $\rightarrow$
					\item What actually \alert{determines stock prices} and their movement?
				\end{itemize}
			}
		\end{column}
		\begin{column}{0.4\linewidth}
			\pause[1]
			\href{https://www.smbc-comics.com/comic/markets}{\includegraphics<handout:0>[width=\linewidth]{pics/1592665375-20200620}}
		\end{column}
	\end{columns}
\end{frame}
...
\begin{frame}{Price determinants}
	...
	\begin{itemize}
		...
		\item traders have \structure<1>{different valuations} \only<1>{(\alert{\textbf{why?}})} $\rightarrow$ prices clear demand and supply
	\end{itemize}
	\pause 
...
\begin{frame}{Types of information}
	\begin{itemize}
		...
		\end{itemize}
		\pause
		\item Reminder: we draw a wedge between \alert{legal} and \structure{academic} ($\leftarrow$ \structure{our}) definitions of \textbf{private information}
		\begin{itemize}
...
\begin{frame}{Types of informational efficiency}
	There are different kinds of \structure{price efficiency}:
	\begin{itemize}
...
%			\href{https://www.smbc-comics.com/?id=3379}{\includegraphics[width=\linewidth]{pics/20140603}}
%		\end{column}
%		\begin{column}{0.4\linewidth}
%			\begin{enumerate}
%				\item sorry. 
%				\item find a mistake in this one. 
%				\item here's another one: \url{https://www.smbc-comics.com/comic/efficient}
%			\end{enumerate}
%		\end{column}
%	\end{columns}
%\end{frame}
...
\begin{frame}{Asset value}
	\begin{itemize}
		...
		\item \structure{Information}: $\Omega_t$ captures the market's (public) knowledge at time $t$: 
		\structure{$\Omega_{t+1}=(\Omega_t, I_{t+1})$} ($I_{t+1}$ is the new info + assume perfect memory). 
...
\begin{frame}{GM85: Model (1)}
	...
	\begin{itemize}
		\item \structure{Speculator/informed trader} (probability $\pi$): has private information about $v$.
		\begin{itemize}
