...
\begin{frame}{More on algo-trading}
	\begin{itemize}
		...
		\begin{itemize}
			\item I also put on absalon a 2020 SEC report on algo-trading that gives a bird's-eye overview
		\end{itemize}
...
\begin{frame}{Model: high-frequency trading}
	\begin{itemize}
		...
		\end{itemize}
		\item \textbf{Timing} (within period $\tau=1$):
		\begin{enumerate}
			\item Each institution $i$ observes $y_{i}$, and if fast, observes $v$
			\item Each institution $i$ finds a trading opportunity or not. If yes, chooses whether to buy/sell/abstain (one-unit trades only): $d_i \in \{-1,0,1\}$.
			\item Liquidity providers execute order $d_i$ at price $\mathbb{E}(v|d_i)$
			(implicit assumption of market maker competition + no aggregate order flow transparency)
		\end{enumerate}
	\end{itemize}
\end{frame}
...
\begin{frame}{Multiple equilibria}
	\begin{itemize}
		...
		\end{itemize}
		\item \textbf{Assumption}:  $\frac{\epsilon}{2} < \delta < \epsilon$: both $v$ and $y$ matter; $v$ more so. Then
		\[
		V^F_{GH}>V^S_H>V^F_{GL}>\mu>V^F_{BH}>V^S_L>V^F_{BL},
		\]
		where $V^i_j$ is the value of a type-$i$ institution with type-$j$ information.
...
\begin{frame}{When do we have multiple equilibria?}
	\begin{itemize}
		\item \alert{P3 equilibrium}: Always exists. If dealers believe only fast institutions trade $\rightarrow$  high $a$. But then only optimal for fast institutions to trade.
		\item \textbf{Proposition}: \alert{P1 equilibrium} exists if $\alpha<\alpha_{P1} \equiv \frac{\rho(\epsilon-\delta)}{\rho(\epsilon-\delta)+\delta}$. Proof. 
		\begin{enumerate}
		...
		\end{enumerate}
		\item Similarly, can find values of $\alpha$ s.t. \alert{P2 equilibrium} exists
...
\begin{frame}{Institution gain (Fast Institutions)}
	\begin{itemize}
		...
		& = \frac{(1-\alpha)\rho}{\alpha+(1-\alpha)\rho} \epsilon \equiv \pi_F(\alpha)
		\end{align*}
		\item Notice $\pi_F'(\alpha)<0$.
	\end{itemize}
\end{frame}
...
\begin{frame}{Institution gain (Slow Institutions)}
	\begin{itemize}
		...
		& = \rho \left( \delta -  \frac{\alpha}{\alpha+(1-\alpha)\rho} \epsilon \right) \equiv \pi_S(\alpha)
		\end{align*}
		\item Notice $\pi_S'(\alpha)<0$.
	\end{itemize}
\end{frame}
...
\begin{frame}{Institution gain}
	\begin{itemize}
		\item Both $\pi_F(\alpha)$ and $\pi_S(\alpha)$ are decreasing in $\alpha$: all trader types lose when proportion of fast traders increases
		...
		\begin{itemize}
			\item Note: $\pi_F(\alpha)-\pi_S(\alpha) > 0$ is independent of $\alpha$
		\end{itemize}
...
\begin{frame}{Endogenous acquisition of technology}
	\begin{itemize}
		...
		\begin{align*}
		& \pi_F(\alpha) \cdot n - C \geq \pi_S(\alpha) \cdot n  \\
		\Leftrightarrow \, & n \geq \frac{C}{\pi_F(\alpha)-\pi_S(\alpha)} \equiv n(\alpha)
		\end{align*}
	\end{itemize}
\end{frame}
...
\begin{frame}{Endogenous acquisition of technology (3)}
	\begin{itemize}
		...
		\begin{itemize}
			\item \emph{more trading opportunities} -- personal and social benefit
			\item \emph{pvt info about $v$} -- personal benefit, social cost (worse prices for everybody)
		\end{itemize}
	\end{itemize}
\end{frame}
...
\begin{frame}[noframenumbering]{\citet*{budish_high-frequency_2015}}
	...
	ES and SPY are the two largest instruments tracking S\&P500. In theory perfectly correlated. Panel (a) shows a trading day.
	\center
...
\begin{frame}[noframenumbering]{\citet*{budish_high-frequency_2015}}
	ES and SPY are the two largest instruments tracking S\&P500. In theory perfectly correlated. Panel (b) shows a trading hour.
	\center
...
\begin{frame}[noframenumbering]{\citet*{budish_high-frequency_2015}}
	ES and SPY are the two largest instruments tracking S\&P500. In theory perfectly correlated. Panel (c) shows a trading minute.
	\center
...
\begin{frame}[noframenumbering]{\citet*{budish_high-frequency_2015}}
	ES and SPY are the two largest instruments tracking S\&P500. In theory perfectly correlated. Panel (d) shows a high-frequency breakdown.
	\center
...
\begin{frame}{HFT Conclusion}
	\begin{itemize}
		\item Effects of speed are similar to those of informed trading
...
\begin{frame}[label=problems]{Why not?}
	\begin{itemize}
		...
		\begin{itemize}
			\item Visa: 150m tx/day; Bitcoin: 300k tx/day (15.02.21)
		\end{itemize}
		...
		\item \alert{Clearing and settlement}: without a trusted mediator, counterparty and security risks instensify \hyperlink{apesgone}{\beamerbutton{pic}}
		\item \alert{No transparency requirements}: it is more difficult to enforce disclosure of financial info by firms/coin issuers
	\end{itemize}
\end{frame}
...
\begin{frame}{Revealed preference}
	\begin{itemize}
		...
		\end{itemize}
		\item Signal founder has a \href{https://moxie.org/2022/01/07/web3-first-impressions.html}{\underline{nice post}} about it.
	\end{itemize}
\end{frame}
...
\begin{frame}<handout:0>[label=apesgone]
	\centering
	\includegraphics[width=0.7\paperwidth]{pics/apes}
	
	An example of a problem from a lack of trusted intermediary \hyperlink{problems}{\beamerbutton{back}}
\end{frame}
