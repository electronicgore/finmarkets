...
\begin{frame}{Glosten model}
	...
	\begin{itemize}
		\item To show this, use LOB model of section 6.2 with display cost $C>0$, tick size $\Delta >0$, \structure{no adverse selection} (asset value is $\mu$, only the non-execution risk is relevant)
...
\begin{frame}{Glosten model: equilibrium}
	\begin{itemize}
		\item If both markets active ($q^I>0$ and $q^E>0$) then the zero-profit condition holds for both
		\item Then the probabilities in (7.11) and (7.12) must be identical, implying
		\[
		\left(1-\gamma + \frac{\gamma}{2}\right)(1-F(q^I)) = \frac{\gamma}{2}(1-F(q^E))
		\]
		\item When $\gamma < 1$, this implies $F(q^I) > F(q^E)$, so $q^I > q^E$
...
\begin{frame}{Glosten model: comparing depth}
	We can relate $q^I, q^E$ and depth $q^C$ in a consolidated mkt as follows. (Recall $\bar{q}=q^I+q^E$).
	\begin{itemize}
		\item  The pricing equation also holds with a single platform. This implies $\mathbb{P}(x_I \geq q^I) = \mathbb{P}(x \geq q^C)$ 
		\item By (7.11): $\mathbb{P}(x_I \geq q^I) = \frac{1}{2} \Big[$ weighted avg. of $1-F(q^I)$ and $1-F(\bar{q}) \Big]$
		\item But with a single platform: $\mathbb{P}(x \geq q^C)=\frac{1}{2} [1-F(q^C)]$
		\item Hence (since zero-profit cond implies these probabilities are equal):
		\[
		q^I<q^C<q^I+q^E = \bar{q}.
		\]
		\item \structure{Fragmented} LOB market is \structure{deeper} than consolidated LOB market
	\end{itemize}
\end{frame}
